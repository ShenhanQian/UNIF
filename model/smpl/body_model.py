...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        if '.npz' in bm_fname:
            ...
            for k, v in smpl_dict.items():
                if isinstance(v, chumpy.ch.Ch):
                    smpl_dict[k] = np.array(v)
                if isinstance(v, scipy.sparse.csc.csc_matrix):
                    smpl_dict[k] = v.toarray()
        else:
        ...
        self.comp_register('init_v_template', torch.tensor(smpl_dict['v_template'][None], device=device, dtype=dtype), persistent=persistant_buffer)
        ...
        self.comp_register('shapedirs', torch.tensor(np.array(shapedirs), device=device, dtype=dtype), persistent=persistant_buffer)
        ...
        if self.model_type == 'smplx':
            ...
            self.comp_register('exprdirs', torch.tensor(exprdirs, device=device, dtype=dtype), persistent=persistant_buffer)

            expression = torch.tensor(np.zeros((1, num_expressions)), device=device, dtype=dtype)
        ...
        if self.use_dmpl:
            ...
            self.comp_register('dmpldirs', torch.tensor(dmpldirs, device=device, dtype=dtype), persistent=persistant_buffer)

        # Regressor for joint locations given shape - 6890 x 24
        self.comp_register('J_regressor', torch.tensor(smpl_dict['J_regressor'], device=device, dtype=dtype), persistent=persistant_buffer)

        # Pose blend shape basis: 6890 x 3 x 207, reshaped to 6890*30 x 207
        if use_posedirs:
            posedirs = smpl_dict['posedirs']
            posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
            self.comp_register('posedirs', torch.tensor(posedirs, device=device, dtype=dtype), persistent=persistant_buffer)
        else:
            self.posedirs = None
        ...
        self.comp_register('weights', torch.tensor(weights, device=device, dtype=dtype), persistent=persistant_buffer)

        self.comp_register('init_trans', torch.zeros((1,3), device=device, dtype=dtype), persistent=persistant_buffer)
        # self.register_parameter('trans', nn.Parameter(trans, requires_grad=True))

        # root_orient
        # if self.model_type in ['smpl', 'smplh']:
        self.comp_register('init_root_orient', torch.zeros((1,3), device=device, dtype=dtype), persistent=persistant_buffer)

        # pose_body
        if self.model_type in ['smpl', 'smplh', 'smplx']:
            self.comp_register('init_pose_body', torch.zeros((1,63), device=device, dtype=dtype), persistent=persistant_buffer)
        elif self.model_type == 'animal_horse':
            self.comp_register('init_pose_body', torch.zeros((1,105), device=device, dtype=dtype), persistent=persistant_buffer)
        elif self.model_type == 'animal_dog':
            self.comp_register('init_pose_body', torch.zeros((1,102), device=device, dtype=dtype), persistent=persistant_buffer)

        # pose_hand
        if self.model_type in ['smpl']:
            self.comp_register('init_pose_hand', torch.zeros((1,1*3*2), device=device, dtype=dtype), persistent=persistant_buffer)
        elif self.model_type in ['smplh', 'smplx']:
            self.comp_register('init_pose_hand', torch.zeros((1,15*3*2), device=device, dtype=dtype), persistent=persistant_buffer)
        elif self.model_type in ['mano']:
            self.comp_register('init_pose_hand', torch.zeros((1,15*3), device=device, dtype=dtype), persistent=persistant_buffer)

        # face poses
        if self.model_type == 'smplx':
            self.comp_register('init_pose_jaw', torch.zeros((1,1*3), device=device, dtype=dtype), persistent=persistant_buffer)
            self.comp_register('init_pose_eye', torch.zeros((1,2*3), device=device, dtype=dtype), persistent=persistant_buffer)

        self.comp_register('init_betas', torch.zeros((1,num_betas), device=device, dtype=dtype), persistent=persistant_buffer)

        if self.use_dmpl:
            self.comp_register('init_dmpls', torch.zeros((1,num_dmpls), device=device, dtype=dtype), persistent=persistant_buffer)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        verts, Jtr, Jrot = lbs(betas=shape_components, pose=full_pose, v_template=v_template,
        ...
        res['Jrot'] = Jrot
        res['Jpose'] = full_pose.reshape(batch_size, -1, 3)
